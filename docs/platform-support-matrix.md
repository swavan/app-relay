...
- macOS keyboard input sends text and a conservative set of key commands through
  System Events. macOS pointer input targets native selected windows using
  `macos-window-{session_id}-{native_id}` ids and delivers move, primary,
  secondary, middle, scroll, and drag events through osascript. Non-native
  selected windows fail pointer delivery conservatively instead of pretending to
  target the window.
