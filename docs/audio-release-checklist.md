...
- Confirm the test-only client playback runtime starts media telemetry for active streams and clears telemetry when readiness is downgraded.
...
- Confirm production stream lifecycle calls native runtime start/stop hooks for capture, playback, client microphone capture, and server-side microphone injection, while unavailable OS backends continue to report explicit planned-native failures.
...
- Confirm Linux servers built with `pipewire-capture` can opt into command-backed PipeWire capture with `APPRELAY_PIPEWIRE_CAPTURE=1`; default feature builds without that environment flag must still report the unavailable adapter boundary.
